    ...
    try {
      ...
      if (user) {
        ...
        if (weight) {
          ...
          res.locals.user.weight_history.push({
            _id: newWeight._id,
            weight: weight,
          });
    ...
    try {
      ...
      if (user) {
        ...
        if (itemId && newWeight) {
          ...
          const index = res.locals.user.weight_history.findIndex(
    ...
    try {
      ...
      if (user) {
        ...
        if (itemId) {
          ...
          res.locals.user.weight_history =
            res.locals.user.weight_history.filter(
              (item) => item._id !== itemId
            );
          res.redirect(`/users/${user._id}/tracker`);
        } else {
    ...
    try {
      ...
      if (user) {
        ...
        if (itemId && status && repetitions && exercise) {
          ...
          const index = res.locals.user.plan_history.findIndex(
            (item) => item._id === itemId
          );
          res.locals.user.plan_history[index] = {
            _id: itemId,
            exercise,
            repetitions,
            status,
    ...
    try {
      ...
      if (user) {
        ...
        if (itemId) {
          ...
          res.locals.user.plan_history = res.locals.user.plan_history.filter(
            (item) => item._id !== itemId
          );
          res.redirect(`/users/${user._id}/tracker`);
        } else {
